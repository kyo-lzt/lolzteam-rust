...
/// Collect the set of component schema original names that are actually referenced
/// by response types or parameter types.
...
fn collect_used_component_schemas(parsed: &ParseResult) -> Vec<String> {
	...
	for group in &parsed.groups {
		for method in &group.methods {
			...
			// Also scan parameter types (path, query, body) for component refs
			for param in method
				.path_params
				.iter()
...
fn collect_component_refs_from_response(
	...
	for field in fields_iter {
		extract_component_from_type(&field.rust_type, rust_to_original, used);
	}
}

/// Strip wrapper types (Vec<>, Option<>, HashMap<String,>) from a Rust type string
/// and check if the inner type is a known component schema.
fn extract_component_from_type(
	rust_type: &str,
	rust_to_original: &BTreeMap<String, String>,
	used: &mut std::collections::BTreeSet<String>,
) {
...
fn emit_component_schema(out: &mut String, name: &str, schema: &Value, root: &Value) {
	...
	writeln!(out, "#[derive(Debug, Clone, Default, Deserialize)]").unwrap();
	...
	for (prop_name, prop_schema) in properties {
		...
		if required {
			let has_default = rust_type_has_default(&rust_type);
			let is_struct = is_struct_type(&rust_type);
			// Struct-typed required fields: use #[serde(default)] with bare type.
			// The struct derives Default, so serde can fill missing fields.
			if is_struct {
				if needs_rename {
					writeln!(out, "\t#[serde(rename = \"{prop_name}\", default, deserialize_with = \"deserialize_null_default\")]").unwrap();
				} else {
					writeln!(
						out,
			...
			} else if needs_rename && has_default {
				writeln!(out, "\t#[serde(rename = \"{prop_name}\", default, deserialize_with = \"deserialize_null_default\")]").unwrap();
				writeln!(out, "\tpub {field_name}: {rust_type},").unwrap();
			...
			// Optional fields: use lenient deserializer to handle type mismatches
			if needs_rename {
				writeln!(out, "\t#[serde(rename = \"{prop_name}\", default, deserialize_with = \"deserialize_lenient_option\")]").unwrap();
			} else {
				writeln!(
					out,
...
fn emit_response_struct(out: &mut String, struct_name: &str, fields: &[ResponseFieldDef]) {
	...
	for field in fields {
		...
		if field.required {
			let has_default = rust_type_has_default(&field.rust_type);
			let is_struct = is_struct_type(&field.rust_type);
			// Struct-typed required fields: use #[serde(default)] with bare type.
			// The struct derives Default, so serde can fill missing fields.
			if is_struct {
				if needs_rename {
					writeln!(out, "\t#[serde(rename = \"{api_name}\", default, deserialize_with = \"deserialize_null_default\")]").unwrap();
				} else {
					writeln!(
						out,
						"\t#[serde(default, deserialize_with = \"deserialize_null_default\")]"
			...
			// Optional fields: use lenient deserializer to handle type mismatches
			if needs_rename {
				writeln!(out, "\t#[serde(rename = \"{api_name}\", default, deserialize_with = \"deserialize_lenient_option\")]").unwrap();
			} else {
				writeln!(
					out,
					"\t#[serde(default, deserialize_with = \"deserialize_lenient_option\")]"
...
/// Strip markdown bold markers (`**text**`) to plain text for rustdoc.
fn strip_markdown_bold(s: &str) -> String {
	s.replace("**", "")
}

/// Emit rustdoc lines for a method (summary + description).
fn emit_method_doc(out: &mut String, method: &MethodDef) {
	let summary = method.summary.as_deref().map(strip_markdown_bold);
	let description = method.description.as_deref().map(strip_markdown_bold);
	...
	if let Some(ref desc) = description {
		// Separate summary and description with a blank doc line
		if summary.is_some() {
			writeln!(out, "\t///").unwrap();
		}
...
/// Emit a doc comment for a struct field (description + default).
fn emit_field_doc(out: &mut String, param: &ParamDef, indent: &str) {
	let desc = param.description.as_deref().map(strip_markdown_bold);
	let default = param.default_value.as_deref();

	match (desc, default) {
		(Some(d), Some(def)) => {
			// Single-line if description fits
			let first_line = d.lines().next().unwrap_or("");
			let remaining: Vec<&str> = d.lines().skip(1).collect();
			if remaining.is_empty() {
				writeln!(out, "{indent}/// {first_line} Default: {def}").unwrap();
			} else {
				writeln!(out, "{indent}/// {first_line}").unwrap();
				...
				writeln!(out, "{indent}///").unwrap();
				writeln!(out, "{indent}/// Default: {def}").unwrap();
			}
		}
		(Some(d), None) => {
			for line in d.lines() {
...
/// Check if a Rust type implements Default (safe for #[serde(default)]).
fn rust_type_has_default(rust_type: &str) -> bool {
	matches!(
		rust_type,
		"String" | "i64" | "i32" | "u64" | "u32" | "f64" | "f32" | "bool" | "serde_json::Value"
	) || rust_type.starts_with("Vec<")
		|| rust_type.starts_with("Option<")
		|| rust_type.starts_with("HashMap<")
		|| rust_type.starts_with("std::collections::HashMap<")
}

/// Check whether a response field type is a custom struct (not a primitive, Vec, HashMap, etc).
/// Such types need `Option<T>` wrapping even when required, because the API may return null.
fn is_struct_type(rust_type: &str) -> bool {
	!rust_type_has_default(rust_type)
		&& !rust_type.starts_with("Option<")
		&& rust_type != "StringOrInt"
}
